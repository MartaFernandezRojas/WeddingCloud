...
class Listas extends Component {
    ...
    onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            let state = { familiaNovio };

            if (source.droppableId === 'droppable2') {
                state = { selected: familiaNovio };
            }
                ...
                <Droppable droppableId="droppable">
                    {(provided, snapshot) => (
                        <div
                            ...
                            {this.state.familiaNovio.map((item, index) => (
                                <Draggable
                                    key={item.id}
                                    draggableId={item.id}
                                    index={index}>
                ...
                <Droppable droppableId="droppable2">
                    {(provided, snapshot) => (
                        <div
                            ref={provided.innerRef}
                            style={getListStyle(snapshot.isDraggingOver)}>
                            {this.state.familiaNovia.map((item, index) => (
                                <Draggable
                                    key={item.id}
                                    draggableId={item.id}
                                    index={index}>
...
ReactDOM.render(<Listas />, document.getElementById('root'));
